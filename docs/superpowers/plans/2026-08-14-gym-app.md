...
既存の `docs/` と `.git/` を上書きしないこと。`npm create vite` が既存ファイルの扱いを尋ねた場合は、既存ファイルを残す選択をする。
...
`Tバーロウ` の正規化名が `t バーロウ` ではなく `tバーロウ` になるよう注意する。`normalizeExerciseName` は小文字化と空白除去のみを行うため、`Tバーロウ` → `tバーロウ` が正しい。上記SQLの該当行を `('Tバーロウ', 'tバーロウ', 'back', true),` に修正すること。
